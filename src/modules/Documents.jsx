...
// Normalize documents.processing_type (live column) to a DOC_TYPES key.
// documents.groq_classification is mostly NULL in live data; processing_type is the
// reliable classification field set by the Doc Processor. Keep this map in sync
// with the recipe's processing_type vocabulary.
const PROCESSING_TYPE_TO_DOC_TYPE = {
  sf_comp_recap:                              "comp_recap",
  sf_deduction_stmt:                          "comp_recap",
  paychex_payroll:                            "payroll_export",
  bank_statement_operating:                   "bank_statement",
  bank_statement_business_cc:                 "bank_statement",
  chase_operating_stmt:                       "bank_statement",
  cpa_balance_sheet_monthly:                  "tax_document",
  cpa_income_statement_monthly_compendium:    "tax_document",
  cpa_income_statement_annual:                "tax_document",
  cpa_general_ledger_annual:                  "tax_document",
  archive_zip_bank_statements_2026:           "bank_statement",
};
const classifyDocType = (doc) =>
  PROCESSING_TYPE_TO_DOC_TYPE[doc?.processing_type] || "other";
...
const DocCard = ({ doc, onNavigate }) => {
  ...
  const dt  = DOC_TYPES[classifyDocType(doc)] || DOC_TYPES.other;
      ...
      <div style={{ display:"flex", alignItems:"center", gap:10, padding:"11px 14px", cursor:"pointer" }} onClick={() => setExpanded(e=>!e)}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:6, flexWrap:"wrap" }}>
            <DocTypeBadge type={classifyDocType(doc)} />
            ...
            <span style={{ fontSize:10, color:T.slate400 }}>{doc.uploaded_at}</span>
          </div>
        </div>
...
const DocumentsOverview = ({ documents, onNavigate }) => {
  ...
  const byType = Object.keys(DOC_TYPES).map(type => ({
    type, count:documents.filter(d => classifyDocType(d) === type).length,
  })).filter(t => t.count > 0);
            ...
            return (
              <div key={doc.id} style={{ display:"flex", alignItems:"center", gap:10, padding:"8px 0", borderBottom:i<recent.length-1?`1px solid ${T.slate100}`:"none" }}>
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  ...
                  <div style={{ fontSize:10, color:T.slate400 }}>{doc.uploaded_at}</div>
                </div>
...
const DocumentLibrary = ({ documents }) => {
  ...
  const filtered = useMemo(() => documents.filter(d => {
    if (typeFilter   !== "all" && classifyDocType(d) !== typeFilter)   return false;
